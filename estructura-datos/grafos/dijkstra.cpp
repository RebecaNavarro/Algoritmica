...
void dijkstra(int verticeInicial)
{
   ...
   // cola de prioridad priority_queue <pair<int, int>, greater<> > cola; mayor a menor
   //esto hace el multiset
   // va ordenar de mayor a menor 
    multiset<pair<int, int> > colaPrioridad; // log(n)sk menor a mayor igual que la linea 18
    //puede ser un set porque solo nos importa el peso menor en total
    distancia[verticeInicial] = 0;
    colaPrioridad.insert(make_pair(0, verticeInicial));
    while (!colaPrioridad.empty())
    {
        pair<int, int> verticeActual = *colaPrioridad.begin(); //* para dar el valor
        colaPrioridad.erase(colaPrioridad.begin()); // pop() lo borra al que acabamo e recibir su valor
